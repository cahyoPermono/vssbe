...
app.post("/vehicle/apiAddDevice.action", async (c) => {
  const { deviceID, deviceName, deviceType, channelName, token, nodeGuid } =
    await c.req.json();
...
app.post("/vehicle/apiModifyDevice.action", async (c) => {
  const { deviceID, deviceName, channelName, deviceType, token } =
    await c.req.json();
  const response = await fetch(
    `${VSS_API_URL}/vehicle/apiModifyDevice.action`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
app.post("/vehicle/apiRemoveDevice.action", async (c) => {
  ...
  const response = await fetch(
    `${VSS_API_URL}/vehicle/apiRemoveDevice.action`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
